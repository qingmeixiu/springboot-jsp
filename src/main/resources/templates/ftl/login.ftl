...
<body class="gray-bg">
     				  ...
     				  var param = $("#loginForm").serialize(); 
		        	 	...
		        	 	$.ajax({
							...
							success : function(result) { 
								 maskhide();
								if( result.code == 200 ){
									showLoginMsg('登入成功','success');
									window.location.href= result.data
								}else{
									showLoginMsg(result.msg,'warning');				
								}
	...
	function showLoginMsg(stext,type){
	
		Command: toastr[type](stext)
